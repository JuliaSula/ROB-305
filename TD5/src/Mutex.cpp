...
Mutex::Mutex(bool isInversionSafe)
{
	...
	if(isInversionSafe)
	{
		cout<<" [Mutex] InversionIsSafe "<<endl;	
...
Mutex::Lock::Lock(Mutex &m, int priority): Mutex::Monitor::Monitor(m)
{
	m.lock(priority);
}
